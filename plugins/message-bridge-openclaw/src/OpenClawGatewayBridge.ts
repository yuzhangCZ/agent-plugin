...
export class OpenClawGatewayBridge {
  ...
  private syncStatusFromBridgeRuntime(): void {
    ...
    let nextRuntimePhase: NonNullable<MessageBridgeStatusSnapshot["runtimePhase"]>;
    if (this.runtimePhaseOverride !== null && this.runtimePhaseOverride !== undefined) {
      nextRuntimePhase = this.runtimePhaseOverride;
    } else {
