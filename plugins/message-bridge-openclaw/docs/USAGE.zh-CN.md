...
- 运行时宿主版本：`>=2026.3.24`
- 统一 CLI 与 bundle 安装共享同一份纯插件分发包，不再发布 installer/bin
...
- 正式私有 npm 安装
  - 对外唯一入口固定为 `npx @wecode/skill-plugin-cli install --host openclaw`
...
1. 正式安装：`npx --registry ... @wecode/skill-plugin-cli install --host openclaw ...`
...
- `channel` 默认值为 `openclaw`
- `deviceName` 来自 `os.hostname()`
- `toolVersion` 来自插件运行时包版本
- `macAddress` 来自首个可用本地网卡，取不到时为空串 `""`

当前插件内置已知 `channel` 仅 `openclaw`。若注入其他值，会记录 `runtime.register.channel.unknown` 警告日志，但不会阻断连接。
