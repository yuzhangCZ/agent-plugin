...
**Related:** `../README.md`, `../quality/validation.md`, `../topics/mb-p0-first-chunk-stability.md`, `../topics/mb-p0-permission-bridge-requirements.md`
...
- **阶段一诊断增强**
  ...
  - `../quality/validation-audit.md` 验证审计文档
...
- 新增 `../quality/validation-audit.md` 验证审计文档
...
- 状态、探活与诊断
  ...
  - probe / runtime 连接竞争与 `duplicate_connection` 修复归档见 `../topics/mb-p0-probe-runtime-connection-race.md`
  - **v0.6 新增诊断增强**：
    - duplicate_connection 抑制逻辑（避免健康运行时误报）
...
- `../quality/validation-audit.md`（v0.6 新增验证审计文档）
- `../design/protocol-sequence.md`
...
- 用干净新会话做最小回复验证
- 明确模型、超时、首块延迟之间的关系
- 把 `../quality/validation.md` 纳入固定回归流程
- **v0.6 已完成**：
...
- 继续维护 `dist/` 目录安装方式
- 继续维护 bundle 目录安装方式
- 维护 `README.md`、`../operations/usage.md`、`../quality/validation.md`
- 评估是否需要正式发布或安装渠道
- **v0.6 已完成**：
  - 新增 `validate:skill-relay` 脚本命令
  - 新增 `scripts/skill-relay-live-check.mjs` skill relay 实时检查脚本
...
4. 提交四：形成固定回归矩阵
   - 把 `../quality/validation.md` 里的阶段一、阶段二步骤沉淀为持续执行清单
