...
export class ConfigResolver {
  ...
  private loadEnvConfig(): Partial<BridgeConfig> {
    ...
    if (process.env.BRIDGE_GATEWAY_RECONNECT_JITTER) {
      reconnect.jitter = process.env.BRIDGE_GATEWAY_RECONNECT_JITTER;
    }
    if (process.env.BRIDGE_GATEWAY_RECONNECT_MAX_ELAPSED_MS) {
      reconnect.maxElapsedMs = parseInt(process.env.BRIDGE_GATEWAY_RECONNECT_MAX_ELAPSED_MS, 10);
    }
