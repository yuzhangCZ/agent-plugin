...
- 回包 `tool_error.error` 采用中文友好文案，由 `ToolErrorMessageCatalog` 统一维护：`unsupported_action` 拼接实际 action 值（`暂不支持该操作类型，请检查版本后重试 (unsupported_action: {action})`）；`invalid_field_value` 拼接 `violation.field`（`请求格式异常，请稍后重试 (invalid_field_value: {field})`，segment 缺失时降级为 `(invalid_field_value)`）；其他 code 走兜底文案 `请求处理异常，请稍后重试`
