...
结论：
1. 第一阶段采用 `forwardToProvider`，继续保留 `reject` 作为默认兼容策略。
2. 第一阶段不实现 SDK 侧 run 输出缓存队列；开启 `forwardToProvider` 的宿主必须保证同一 `toolSessionId` 下多 active run 的输出对 gateway/前端可消费。
3. 本阶段优先解决 SDK 前置 busy 拦截导致输入无法到达宿主的问题；输出串行强保证留给 `serializedOutput` 后续方案。
4. 第一阶段 public API 只暴露 `'reject' | 'forwardToProvider'`。
...
sequenceDiagram
    ...
    Note over sdk,provider: 方案 A forwardToProvider：SDK 不排序、不缓存
    gw->>sdk: chat -> run-1
...
| 对象 | 负责 | 不负责 |
|---|---|---|
| SDK | active run 策略判断、`runId` 注册与释放、provider fact 校验与投影、terminal 收口、diagnostics | 宿主队列/打断/合并策略；`forwardToProvider` 下跨 run 输出排序 |
...
2. `forwardToProvider` 下，provider/宿主负责确保 gateway/前端看到的输出可消费。
...
| `StartRequestRunUseCase` | 编排 start request run 用例，包括校验、运行时状态协调、provider 调用和失败收口 | active run 存在时的 `reject` / `forwardToProvider` 策略判断发生在该用例内 |
...
sequenceDiagram
    ...
    alt activeRunChatPolicy = reject 且集合非空
        rt-->>gw: tool_error(run_already_active)
    else activeRunChatPolicy = forwardToProvider 或集合为空
...
2. `forwardToProvider`：同一 `toolSessionId` 已有 active run 时，SDK 仍注册新 `runId` 并调用 `provider.runMessage(input)`；各 run 后续按自己的 `runId` 独立 settle。
3. `forwardToProvider` 只改变“新 chat 是否能到达 provider”，不改变 SDK fact 投影主链路。
...
| 单 active run | `activeRunIds.size === 1` | 当前 `toolSessionId` 只有一个 active request run | `reject` 下拒绝新 chat；`forwardToProvider` 下可追加新 run；`abort_session` 生成 `runIds: [runId]` |
| 多 active run | `activeRunIds.size > 1` | 当前 `toolSessionId` 有多个 active request run | `reject` 下拒绝新 chat；`forwardToProvider` 下继续由宿主调度；`abort_session` 生成全部 active `runIds` |
...
export interface SessionRuntimeRegistry {
  ...
  /**
   * 返回 request run 状态快照。
   * @remarks activeRunIds 用于 start_request_run 的 policy 判断和并发 diagnostics，
   * 并供 abort_execution 生成精确 runIds。
   */
...
`registerRequestRun` 只负责把 `runId` 加入 active run set，不负责判断是否允许并发 run。是否在集合非空时拒绝新 chat，由 `StartRequestRunUseCase` 根据 `activeRunChatPolicy` 决定。`StartRequestRunUseCase` 只调用一次 `getRequestRunState`，因为 policy 判断和并发 diagnostics 共享完整 `activeRunIds` 快照；重复查询会增加开销，并可能让两处逻辑观察到不一致的状态。现有 `acquireRequestRun(...): { ok: false }` 和 `getActiveRequestRunId(...)` 是单 active run 语义，应被集合 API 替代。
...
flowchart TD
    ...
    HasActive -->|是| Policy{"activeRunChatPolicy"}
    Policy -->|reject| Reject["发送 tool_error(run_already_active)"]
    Policy -->|forwardToProvider| Register
...
- 触发：`activeRunIds.length > 0` 且 `activeRunChatPolicy = reject`。
- SDK：不注册新 run，不调用 provider。
- 下游：发送 `tool_error(run_already_active)`。
- 状态：原 active run set 不变。

**新 chat：有 active run + `forwardToProvider`**

- 触发：`activeRunIds.length > 0` 且 `activeRunChatPolicy = forwardToProvider`。
...
`activeRunChatPolicy` 在 runtime 创建时从 `BridgeRuntimeOptions.requestRunPolicy` 归一化；未配置时固定按 `reject` 处理。`activeRunsByToolSessionId` 是 `SessionRuntimeRegistry.requestRun` 的内部实现数据，只表达 SDK 本地 active request run 集合，不代表宿主真实 run 队列。
...
| `requestRunPolicy.activeRunChatPolicy` | `ActiveRunChatPolicy` | active run 存在时新 chat 的处理策略 |
| `'reject'` | `ActiveRunChatPolicy` | 默认兼容策略，保持旧行为，active run 存在时返回 `run_already_active` |
...
2. 未传 `requestRunPolicy.activeRunChatPolicy` 时按 `'reject'` 处理。
...
第一阶段不建议所有宿主统一开启 `forwardToProvider`。`activeRunChatPolicy` 应由宿主装配层按 provider adapter 的真实调度能力选择。

| 宿主 | 策略 | 选择原因 | 影响与要求 |
|---|---|---|---|
| OpenCode / `plugins/message-bridge` | 显式配置 `forwardToProvider` | OpenCode adapter 已有 opencode session 级 run queue、superseded run 收口和 abort 协调，能承接同会话多次 `runMessage`。 | 新 chat 可在 active run 期间进入 provider；宿主侧继续负责排队、替换、abort 和输出可消费性 |
...
如果后续希望 OpenClaw 开启 `forwardToProvider`，需要先支持同一 `sessionKey` 下多个 active run 的输出边界，并明确 `abortSession(runIds)` 的中止范围。

选择逻辑总结：

1. `forwardToProvider` 是基于 provider adapter 能力选择的 SDK 转发策略，不是宿主类型开关。
...
|---|---|---|---|
| `src/public-contract.ts` | 修改 | 对外配置和 provider contract | 新增 `requestRunPolicy.activeRunChatPolicy?: 'reject' | 'forwardToProvider'`；`ProviderAbortSessionInput` 使用 `runIds: string[]` |
| `src/application/create-runtime.ts` | 修改 | 默认配置归一化 | 未配置时归一化为 `reject` |
| `src/application/ports/session-runtime-registry.ts` | 修改 | registry port | `RequestRunState` 改为 active run id 集合；用 `registerRequestRun` / `getRequestRunState` 替代单 run `acquireRequestRun` / `getActiveRequestRunId` |
| `src/infrastructure/registries/InMemorySessionRuntimeRegistry.ts` | 修改 | 内存状态实现 | per-`toolSessionId` 维护 active run set |
| `src/application/usecases/StartRequestRunUseCase.ts` | 修改 | request run 启动策略 | 根据 policy 决定 reject 或 forwardToProvider |
| `src/application/usecases/AbortExecutionUseCase.ts` | 修改 | abort 语义 | 根据 active run set 生成 `runIds: string[]` |
| `plugins/message-bridge/src/runtime/SdkBridgeRuntime.ts` | 修改 | OpenCode 装配策略 | 显式配置 `requestRunPolicy.activeRunChatPolicy = 'forwardToProvider'` |
| `plugins/message-bridge-openclaw/src/OpenClawGatewayBridge.ts` | 不改或显式配置 | OpenClaw 装配策略 | 第一阶段保持默认 `reject`；如显式配置也只能配置为 `reject` |
| `tests/runtime-sdk.test.ts` | 修改 | runtime 行为测试 | 覆盖默认、forwardToProvider、多 run settle、abort |
| `tests/public-api-contract.test.ts` | 修改 | public API contract | 覆盖新增配置类型和非法配置值拒绝 |
| `plugins/message-bridge/tests/unit/*` | 修改 | OpenCode 接入测试 | 覆盖 OpenCode runtime options 显式传入 `forwardToProvider` |
...
|---|---|---|---|
| OpenCode 输出可消费性是否满足产品验收 | `forwardToProvider` 接入风险 | 基于现有 host session FIFO 和 superseded run 收口能力，第一阶段建议开启 | `plugins/message-bridge` 维护者 |
| OpenClaw 何时支持多 active run | OpenClaw 后续是否能开启 `forwardToProvider` | 第一阶段不支持，保持 `reject` | `plugins/message-bridge-openclaw` 维护者 |

## 4. 性能

| 项目 | 是否影响 | 说明 | 风险 | 应对策略 |
|---|---|---|---|---|
| 请求数量 | 是 | `forwardToProvider` 下同会话 active run 期间可产生更多 provider 调用 | provider/宿主压力增加 | 仅 opt-in 开启，由接入方评估 |
...
| 对象 | 影响说明 | 验证方式 |
|---|---|---|
| `BridgeRuntimeOptions` | 新增 `requestRunPolicy.activeRunChatPolicy` | public API contract 测试 |
...
| 对象 | 影响说明 | 风险 | 应对策略 |
|---|---|---|---|
| OpenCode provider adapter | 第一阶段建议开启 `forwardToProvider` | 宿主输出不可消费时影响前端 | 依赖现有 host session FIFO、superseded run 收口和 abort 能力，并补接入测试 |
| OpenClaw provider adapter | 第一阶段保持 `reject` | 若误开 `forwardToProvider`，同 `sessionKey` 多 run 会覆盖本地 active run 索引 | 不传 `requestRunPolicy` 或显式配置 `reject`；待 adapter 支持多 active run 后再评估开启 |
...
| request run 状态快照 | policy、diagnostics 与 abort 的共享状态 | 查询空、单个和多个 active `runId` | `getRequestRunState` 分别返回 `[]`、`[runId]` 和完整 active run ID 快照 |
| active run chat policy | 默认兼容和显式开启 | 分别配置未传、`reject`、`forwardToProvider` | 未配置等同 `reject`；`forwardToProvider` 仅显式开启 |
| abort `runIds` 生成 | abort 语义 | active run set 为 0、1、多个 run | 分别生成 `[]`、`[runId]`、全部 active `runIds` |
| public contract 类型 | 接口定义 | 检查 `BridgeRuntimeOptions` 和 `ProviderAbortSessionInput` | `activeRunChatPolicy` 只允许 `'reject' | 'forwardToProvider'`；`runIds: string[]` 必填 |
| 非法配置拒绝 | 配置解析边界 | 尝试使用非法 `activeRunChatPolicy` | 类型或动态解析不接受，不能静默降级 |
...
| `forwardToProvider` 放行第二个 chat | `G1`、`G3` | active run 未 settle 时发送第二个同会话 `chat` | 调用第二次 `provider.runMessage`，两个 run 使用不同 `runId` |
...
| diagnostics 记录并发 run | 观测要求 | `forwardToProvider` 放行第二个 run | 记录 `concurrent_request_runs_detected`，包含 `toolSessionId`、`newRunId`、`activeRunCount` |
