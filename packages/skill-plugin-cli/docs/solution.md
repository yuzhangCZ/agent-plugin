...
**Related:** [统一安装需求说明](../../../docs/superpowers/specs/2026-04-25-skill-plugin-cli.md), [二维码扫码授权方案设计](../../../docs/design/qrcode-auth-session-solution.md)

## 1. 文档定位

本文基于 [统一安装需求说明](../../../docs/superpowers/specs/2026-04-25-skill-plugin-cli.md)，重写 `skill-plugin-cli` 的统一安装方案，并将正文冻结为开发真源。实现阶段不得再就 CLI contract、主流程顺序、宿主命令、完成语义和基础技术选型做高影响再设计。
...
5. OpenClaw 首版安装命令冻结为 `openclaw plugins install @wecode/skill-openclaw-plugin`，完成前只执行 `openclaw --version` 作为宿主可执行性确认。
6. OpenCode 首版不接管宿主启停；CLI 完成安装后统一输出明确的手动重启提示，该场景允许退出 `0`。
7. OpenClaw 首版不再支持 `--no-restart`；CLI 在可执行性确认通过后不自动执行 `openclaw gateway restart`，改为输出明确的手动执行提示，并要求用户通过实际消息链路确认生效。
...
| 5. 插件安装 | 执行宿主所需插件安装动作 | 执行 `opencode plugin -g -f @wecode/skill-opencode-plugin` | 先执行 `openclaw plugins info skill-openclaw-plugin --json` 探测；若退出码为 `0` 则先执行 `openclaw plugins uninstall skill-openclaw-plugin --force`，再执行安装命令 |
| 6. 插件安装校验 | 校验插件已进入宿主可识别状态 | 以安装命令成功退出且全局配置已更新为首版真源 | 执行 `openclaw plugins info skill-openclaw-plugin --json`，并仅以退出码为真源 |
...
| 9. 结果确认 | 确认宿主结果已进入宿主定义的完成语义 | 执行 `opencode --version` 确认可执行，并输出手动重启提示 | 执行 `openclaw --version` 确认可执行，并输出手动 restart 与实际消息验证提示 |
...
- 安装前探测：`openclaw plugins info skill-openclaw-plugin --json`
- 条件卸载：`openclaw plugins uninstall skill-openclaw-plugin --force`
...
- 结果确认：`openclaw --version`
- 手动生效提示：`openclaw gateway restart`

约束如下：

- 安装前探测只以 `openclaw plugins info skill-openclaw-plugin --json` 的退出码判定是否已安装：`0` 表示已安装，非 `0` 表示未安装；该阶段不依赖错误信息文案。
- 探测命中已安装时，CLI 必须先卸载再安装，并在调用 `uninstall` 之前输出“检测到已安装 OpenClaw 插件，正在卸载后重装。”提示。
- 阶段 6 只认 `openclaw plugins info skill-openclaw-plugin --json` 的退出码为安装校验真源，不把运行态探活混入该阶段。
- 阶段 6 若 `openclaw plugins info skill-openclaw-plugin --json` 非零退出，则直接按安装校验失败收口；退出 `0` 即视为校验通过，不再解析返回 JSON 内容。
- `channels add` 的 `--url` 变为可选；未传时优先复用现有 `channels.message-bridge.gateway.url`，否则使用插件 bundle 默认值链。
- 阶段 9 只以 `openclaw --version` 作为完成前真源检查。
- 可执行性确认通过后，CLI 输出手动执行 `openclaw gateway restart` 的明确提示，并要求用户通过实际消息链路确认 channel 生效。
- 阶段 9 不再承诺自动完成 channel 级探活，也不把 `message-bridge` 运行态可用性作为 CLI 成功收口前置条件。
...
- OpenClaw：
  - `openclaw --version` 可执行性确认成功。
  - CLI 已输出明确的手动 restart 提示与 `openclaw gateway restart` 可执行命令。
  - CLI 已提示用户通过实际消息链路确认 `message-bridge` channel 生效。
...
sequenceDiagram
  ...
  CLI->>OC: openclaw --version
  OC-->>CLI: executable ok
