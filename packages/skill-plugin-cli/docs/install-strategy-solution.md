...
**Related:** [统一安装 CLI 方案](./solution.md), [统一安装需求说明](../../../docs/superpowers/specs/2026-04-25-skill-plugin-cli.md)
...
15. OpenCode 无论 `host-native` 还是 `fallback`，都必须尝试清理全局配置目录下历史源码集成残留 `plugins/message-bridge.plugin.js`。
...
- OpenCode 历史 `plugins/message-bridge.plugin.js` 存在但清理失败：安装继续，但必须输出显式 warning，并提示用户手动删除绝对路径
...
该接口只承载宿主特有清理动作，例如 OpenCode 历史 `plugins/message-bridge.plugin.js` 清理，不代表整个 `Plugin Reference Reconciliation` 都由 adapter 独占。
...
- 历史 `plugins/message-bridge.plugin.js` 必须清理；`host-native` 在宿主安装成功后清理，`fallback` 在本地 path spec 写入后清理，二者都在安装校验前完成。
...
cleanup 目标文件固定为 `<resolved-config-dir>/plugins/message-bridge.plugin.js`，warning 中必须展示该解析后的绝对路径。
...
- 受控历史残留文件为 `<resolved-config-dir>/plugins/message-bridge.plugin.js`
...
安装校验前提包含 OpenCode 历史 `plugins/message-bridge.plugin.js` cleanup 已经执行；cleanup 失败只产出 warning，不改变安装校验真源。
...
- OpenCode 历史残留 `plugins/message-bridge.plugin.js` 清理失败时必须输出显式 warning
...
sequenceDiagram
  ...
  alt host-native
    ...
    CLI->>CFG: cleanup legacy plugins/message-bridge.plugin.js
...
- 本专题文档是安装策略真源
- 现有 [`packages/skill-plugin-cli/docs/solution.md`](./solution.md) 后续只保留高层引用
- 本次任务不修改总方案文档正文
...
- OpenCode 两种策略都会尝试清理历史 `plugins/message-bridge.plugin.js`
