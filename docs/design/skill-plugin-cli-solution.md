...
| 6. 插件安装校验 | 校验插件已进入宿主可识别状态 | 以安装命令成功退出且全局配置已更新为首版真源 | 执行 `openclaw plugins info skill-openclaw-plugin --json`，且仅在退出码为 `0` 时继续解析 JSON |
...
- 阶段 6 若 `openclaw plugins info skill-openclaw-plugin --json` 非零退出，则直接按安装校验失败收口，不沿用安装前的“非零即未安装”宽松解释。
