...
- 与 `skill-qrcode-auth`、现有兼容 wrapper 的职责分界
- 方案图示表达与图示数量
...
6. OpenCode 首版允许主动触发生效，但完成语义按宿主状态区分：
   - 未运行时：CLI 主动启动 OpenCode，一次成功拉起宿主进程即可作为首版结果确认通过。
   - 已运行时：CLI 不接管重启，只输出明确重启提示，该场景仍允许退出 `0`。
7. OpenClaw 首版不再支持 `--no-restart`；CLI 在 probe 通过后仍执行 `openclaw gateway restart` 作为稳定化动作，但 restart 失败只降级为 warning。
...
- wrapper 负责把旧入口参数手工映射到统一 CLI 输入，不依赖第三方 CLI 兼容层。
...
| --- | --- | --- | --- |
| 1. 用户启动 CLI | 用户执行统一命令 | 从统一 bin 或 OpenCode wrapper 进入 | 从统一 bin 或 OpenClaw wrapper 进入 |
...
| 9. 结果确认 | 确认宿主结果已进入宿主定义的完成语义 | 未运行时主动启动 OpenCode；已运行时输出明确重启提示并以该提示收口 | 先执行 `openclaw channels status --channel message-bridge --probe --json` 确认可用，再执行 `openclaw gateway restart` 作为稳定化动作 |
| 10. 结束收口 | 输出终态摘要并退出 | 输出 OpenCode 对应分支的完成提示 | 输出 OpenClaw probe 结果；若 restart 失败则在成功摘要中附带 warning |
...
1. 若未检测到 OpenCode 正在运行，CLI 主动启动一次 OpenCode。
2. 首版以宿主进程成功拉起作为结果确认通过，不再额外要求日志、缓存或运行态插件探针。
3. 若检测到 OpenCode 已在运行，CLI 不直接管理重启。
4. CLI 输出“需用户重启 OpenCode 以确认新插件与配置生效”的明确提示。
5. 该场景仍允许退出 `0`，并视为 OpenCode 首版完成收口。
...
- probe 通过后，CLI 仍执行 `openclaw gateway restart` 作为首版稳定化动作。
- 阶段 9 不再仅输出“建议执行”的提示；probe 未通过即不得宣告完成。
...
- OpenCode：
  - 若宿主原先未运行，CLI 已成功拉起 OpenCode 进程。
  - 若宿主原先已运行，CLI 已输出明确重启提示并完成成功收口。
- OpenClaw：
  - `openclaw channels status --channel message-bridge --probe --json` 成功返回可用结果。
  - `openclaw gateway restart` 失败不改变已完成状态，但必须输出 warning。
...
packages/skill-plugin-cli/
  ...
  src/
    ...
    cli/
      ...
      command-spec.ts
      compatibility/
        run-opencode-wrapper.ts
        run-openclaw-wrapper.ts
    domain/
      InstallHostKind.ts
      InstallPhase.ts
...
| `cli/compatibility/run-*-wrapper.ts` | 兼容旧入口，不承载业务编排 |
...
- `packages/skill-plugin-cli` 提供统一 bin。
- `plugins/message-bridge/scripts/setup-message-bridge.mjs` 保留为 OpenCode 兼容 wrapper。
- `plugins/message-bridge-openclaw/scripts/install-openclaw-plugin.mjs` 保留为 OpenClaw 兼容 wrapper。
- wrapper 只负责旧入口兼容、参数映射、调用统一 CLI。
- wrapper 不得继续承载二维码认证、仓源配置、安装校验、配置接入等核心业务。
...
sequenceDiagram
  ...
  CLI->>OC: openclaw gateway restart
  alt restart succeeded
    OC-->>CLI: gateway restarted
    CLI-->>U: 输出成功摘要与确认提示
  else restart failed
    OC-->>CLI: restart failed
    CLI-->>U: 输出成功摘要、warning 与后续提示
  end
```
