...
本 B 方案不修改原方案文档，也不调整最大时间窗口。B 方案保持当前默认 `maxElapsedMs = 600000`，即 10 分钟，但改变重连窗口耗时计算口径：从墙钟耗时改为 active elapsed。电脑休眠、系统挂起、事件循环长时间停顿导致的 timer drift 不计入 10 分钟耗尽窗口。
...
因此，若 SDK 已进入重连窗口后电脑休眠超过 10 分钟，唤醒后 `getExhaustedDecision()` 可能直接返回耗尽，进入 `gateway_reconnect_exhausted`，不再继续 reconnect attempt。

### 1.2 需求目标

1. 保持默认最大重连窗口仍为 10 分钟。
2. 休眠、系统挂起、长时间事件循环停顿期间不消耗 10 分钟重连预算。
3. 唤醒后 gateway 已恢复时，SDK 能继续 reconnect attempt 并回到 READY。
4. 在线且 gateway 持续不可达时，仍按 active elapsed 10 分钟耗尽，保留止损边界。
5. 不改变 WebSocket close code 准入规则。

### 1.3 非目标

1. 不将默认 `maxElapsedMs` 调整为 40 分钟。
2. 不完全放开重连窗口限制。
3. 不新增系统级 sleep/wake 事件监听。
4. 不新增半开连接 watchdog。
5. 不扩展 reconnect attempt 中非 close 失败的重试分类。
6. 不修改 gateway wire 业务消息协议。
...
重连窗口仍是 10 分钟，但 10 分钟只统计 SDK 实际可执行重连逻辑的 active elapsed；休眠/挂起导致的 timer drift 计入 suspended duration，并从窗口耗时中扣除。
...
本方案不新增后台任务，不新增轮询，不延长活跃状态下的重连预算。

与直接放开窗口限制相比，本方案功耗更可控：

1. 机器唤醒且 gateway 持续不可达时，仍会在 active elapsed 10 分钟后停止重连。
...
2. `gateway_reconnect_exhausted` 触发语义从墙钟 10 分钟变为 active elapsed 10 分钟。
3. 日志和诊断需要区分 `wallElapsedMs` 与 `activeElapsedMs`。

### 8.3 不影响

1. 默认 `maxElapsedMs = 600000`。
2. retryable / terminal close code 集合。
3. gateway wire 业务消息 schema。
4. OpenCode / OpenClaw provider 命令实现。
...
1. 休眠漂移扣除
   ...
   - 确认 `activeElapsedMs < 600000` 时继续调度 attempt。

2. active elapsed 耗尽
   - 模拟机器保持唤醒，连续重连 active elapsed 达到 10 分钟。
...
最终结论：B 方案建议采用 active elapsed 作为重连窗口耗时口径，保持最大窗口仍为 10 分钟。休眠、系统挂起、长时间事件循环停顿导致的 timer drift 不计入耗尽时间；机器唤醒且 gateway 持续不可达时，仍按 active elapsed 10 分钟耗尽。

取舍原因：

1. 相比把窗口从 10 分钟调整为 40 分钟，B 方案更精准地修复休眠导致墙钟耗尽的问题。
2. 相比完全放开窗口限制，B 方案保留 SDK 内部止损边界，服务端压力和功耗更可控。
3. 方案不依赖系统级 sleep/wake 事件，跨平台行为更稳定。
4. 实现复杂度高于单纯改默认值，需要扩展 policy 端口和时间相关测试。

后续动作：

1. 评审确认是否选择 B 方案替代或补充原 40 分钟方案。
