...
| Test Framework (node:test + c8) | **VALIDATED** | 4 |
...
### Principle
Per `overview.md` §8 and `../product/prd.md` §9: Tests use Node.js native test runner (`node:test`) with c8 for coverage.
...
| Location | Lines | Evidence |
|----------|-------|----------|
| `package.json` | 11 | `"test": "node --test tests/**/*.test.mjs"` - uses node:test |
| `package.json` | 12-14 | Separate scripts for unit/integration/e2e all use `node --test` |
| `tests/unit/example.test.mjs` | 1 | `import { test, describe } from 'node:test'` - native test runner |

#### 7.2 Coverage Tool

| Location | Lines | Evidence |
|----------|-------|----------|
| `package.json` | 15 | `"test:coverage": "c8 --check-coverage --lines 80 --branches 70..."` |
| `package.json` | 25 | `"c8": "^10.1.2"` in devDependencies |
...
2. Uses c8 for coverage with enforced thresholds
...
6. **Test Framework**: node:test with c8 coverage at 80%/70% thresholds
